...
class _CertificateFormScreenState extends State<CertificateFormScreen> {
  ...
  Future<void> _pickFile() async {
    ...
    if (result != null && result.files.first.bytes != null) {
      ...
      // Check if Base64-encoded size is within Firestore limit (~700KB)
      final base64Size = (bytes.length * 4 / 3).floor() / 1024; // Approx Base64 size in KB
      if (base64Size > 700) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('File too large. Please use a file smaller than 700KB.')),
          );
        }
        return;
      }
      ...
      developer.log('No file selected or unsupported format.', name: 'CertificateForm');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Please select a valid PDF or image file')),
        );
      }
    }
  }
  ...
  Future<void> _submitForm() async {
    ...
    if (startDate == null || endDate == null || selectedFileBytes == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select dates and a file')),
      );
      return;
    }
    ...
    try {
      ...
      final base64File = base64Encode(selectedFileBytes!);
      final fileType = selectedFileName!.toLowerCase().endsWith('.pdf') ? 'pdf' : 'image';
      ...
      developer.log('Submission error: $e', name: 'CertificateForm');
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
                      ...
                      children: [
                        ...
                        AnimatedContainer(
                          ...
                          child: ElevatedButton.icon(
                            ...
                            label: const Text('Select Certificate (PDF/Image)'),
